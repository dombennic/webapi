...
var routes = function () {
    ...
    //cart
    router.get('/api/viewcartkfc', function (req, res) {
        db.viewcartkfc(res.locals.user.username, function (err, cart) {
            res.send(cart);
        })
    })

    router.get('/api/viewcartmcd', function (req, res) {
        db.viewcartmcd(res.locals.user.username, function (err, cart) {
            res.send(cart);
        })
    })

    router.get('/api/viewcartbks', function (req, res) {
        db.viewcartbks(res.locals.user.username, function (err, cart) {
            res.send(cart);
        })
    })

    router.get('/api/viewhistorykfc', function (req, res) {
        db.viewhistorykfc(res.locals.user.username, function (err, history) {
            res.send(history);
        })
    })

    router.get('/api/viewhistorymcd', function (req, res) {
        db.viewhistorymcd(res.locals.user.username, function (err, history) {
            res.send(history);
        })
    })

    router.get('/api/viewhistorybks', function (req, res) {
        db.viewhistorybks(res.locals.user.username, function (err, history) {
            res.send(history);
        })
    })

    //favourite
    router.get('/api/viewfav', function (req, res) {
        db.viewfav(res.locals.user.username, function (err, favourite) {
            res.send(favourite);
        })
    })

    // router.get('/api/viewfavmcd', function (req, res) {
    //     db.viewfavmcd(res.locals.user.username, function (err, favourite) {
    //         res.send(favourite);
    //     })
    // })

    // router.get('/api/viewfavbks', function (req, res) {
    //     db.viewcartbks(res.locals.user.username, function (err, favourite) {
    //         res.send(favourite);
    //     })
    // })
    ...
    router.post('/api/favmcd', function (req, res) {
        var data = req.body; // get form data from the HTTP body
        db.searchuserinfav(data.username, function (err, fav) {
            if (fav) {
                res.status(500).send("You already have a favourite! ");
                // alert("You already have a favourite!")
            } else {
                db.addfavouritemcd(data.username, function (err, fav) {
    ...
    router.post('/api/favkfc', function (req, res) {
        var data = req.body; // get form data from the HTTP body
        db.searchuserinfav(data.username, function (err, fav) {
            if (fav) {
                res.status(500).send("You already have a favourite! ");
                ...
                db.addfavouritekfc(data.username, function (err, fav) {
    ...
    router.post('/api/favbks', function (req, res) {
        var data = req.body; // get form data from the HTTP body
        db.searchuserinfav(data.username, function (err, fav) {
            if (fav) {
                res.status(500).send("You already have a favourite! ");
                ...
                db.addfavouritebks(data.username, function (err, fav) {
    ...
    // router.post('/api/favmcd', function (req, res) {
    //     var data = req.body; // get form data from the HTTP body
    //     db.searchuserinfav(data.username, function (err, infav) {
    //         if (infav != null) 
    //         {
    //             alert("You already have a favourite!")
    //         }
    //         else
    //         {
    //             db.addfavouritemcd(data.macdonald, data.kfc, data.burgerking, data.username, function (err, fav)
    //             {
